...
func torrentFileSize(hash string, fileID string) (size int64) {
	...
	// Upstream also checks the anacrolix handle (tor.Torrent != nil) here; the
	// libtorrent fork has no such field — GotInfo() alone covers "metadata not
	// resolved yet" (it waits on the lt handle and reports false without one).
	if !tor.GotInfo() {
		return 0
	}
